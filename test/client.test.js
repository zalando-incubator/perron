...
describe('ServiceClient with faultTolerantRequest', () => {
    let clientOptions;
    ...
    it('should by default send an `accept` application/json header', () => {
        const client = new ServiceClient(clientOptions);
        return client.faultTolerantRequest().then(() => {
            assert.equal(
                requestStub.firstCall.args[0].headers.accept,
                'application/json'
            );
    ...
    it('should open the circuit after 50% from 5 requests failed', (done) => {
        const httpErrorResponse = Promise.resolve({
            statusCode: 500,
            headers: {},
            body: '{}'
        ...
        [ httpErrorResponse, emptySuccessResponse, errorResponse ].forEach((response, index) => {
            requestStub.onCall(index).returns(response);
        });
