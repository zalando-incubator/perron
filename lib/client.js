...
class ServiceClient {
    ...
    constructor(optionsOrUrl) {
        ...
        this.options.retryOptions = Object.assign({
            ...
            // It takes the error as an argument and the function should return a boolean
            // eslint-disable-next-line
            shouldRetry(err) {
                return true;
            },
            // This is called each time a retry is triggered
        ...
        return new Promise((resolve, reject) => operation.attempt((currentAttempt) => {
            this.breaker.run((success, failure) => {
                return requestWithFilters(params, this.options.filters)
                    ...
                    .catch(error => {
                        failure();
                        const willRetry = operation.retry(error);
                        if (!shouldRetry(error) || !willRetry) {
                            reject(error);
                            return;
                        }
