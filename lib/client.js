...
class ServiceClient {
    ...
    constructor(optionsOrUrl) {
        ...
        this.options.retryOptions = Object.assign({
            ...
            transientErrorCheck: (err) => {
                return (err && err.response && err.response.statusCode >= 500) || (err && err.toString().indexOf('timeout') > -1);
            }
    ...
    request(userParams) {
        ...
        return new Promise((resolve, reject) => this.breaker.run(
            (success, failure) => {
                const opts = {
                    retries: this.options.retryOptions.retries,
                    factor: this.options.retryOptions.factor,
                    minTimeout: this.options.retryOptions.minTimeout,
                    maxTimeout: this.options.retryOptions.maxTimeout,
                    randomize: this.options.retryOptions.randomize
                };
                ...
                operation.attempt(() => {
                    return requestWithFilters(params, this.options.filters)
                        ...
                        .catch(error => {
                            failure();
                            if (!this.options.retryOptions.transientErrorCheck(error) || !operation.retry(error)) {
                                reject(error);
                            }
                        });
                });
